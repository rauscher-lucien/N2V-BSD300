...
def print_jpg_filenames(root_folder_path):
    """
    Prints the filenames of JPEG files in the specified folder and its subdirectories.
    
    Parameters:
    - root_folder_path: Path to the root folder containing JPEG image files.
    """
    for subdir, _, files in os.walk(root_folder_path):
        sorted_files = sorted([f for f in files if f.lower().endswith('.jpg')])
